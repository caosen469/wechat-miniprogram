...
describe('listFeed（简版足迹流水）', () => {
  ...
  test('按 createdAt 时间倒序返回，作者昵称头像由服务端 join（前端无二次请求）', async () => {
    sdk.__reset(seed([
      record({ _id: 'r-old', createdAt: hoursAgo(48), text: '两天前' }),
      record({ _id: 'r-new', createdAt: hoursAgo(1), text: '刚发' }),
      record({ _id: 'r-mid', createdAt: hoursAgo(24), text: '昨天' })
    ]))
    const result = await listFeed.main({})
    expect(result.code).toBeUndefined()
    expect(result.records.map(r => r._id)).toEqual(['r-new', 'r-mid', 'r-old'])
    // join 的作者信息直接可渲染
    expect(result.records[0].author).toEqual({ nickname: '小美', avatarUrl: 'cloud://ava-b.jpg' })
  })
  ...
  test('before 游标：只返回 createdAt 早于 before 的记录（ISO 字符串也接受）', async () => {
    sdk.__reset(seed([
      record({ _id: 'r-new', createdAt: hoursAgo(1) }),
      record({ _id: 'r-old', createdAt: hoursAgo(48) })
    ]))
